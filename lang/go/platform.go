...
type playPlatformSoundCode int16

const (
	// sound codes for playPlatformSound()
	_stopping_sound playPlatformSoundCode = iota
	_starting_sound
	_obstructed_sound
...
type platformDefinition struct {
	...
	startingExtension, startingContraction int16
	stoppingExtension, stoppingContraction int16
	obstructedSound, uncontrollableSound   int16
	movingSound                            int16

	keyItemIndex int16

	defaults staticPlatformData

	damage DamageDefinition
}

const maximumPlatformsPerMap = 64
const (
	/* platform types */
	_platform_is_spht_door = iota
...
const ( /* platform speeds */
	_very_slow_platform       = WorldOne / (4 * TicksPerSecond)
	_slow_platform            = WorldOne / (2 * TicksPerSecond)
	_fast_platform            = 2 * _slow_platform
	_very_fast_platform       = 3 * _slow_platform
	_blindingly_fast_platform = 4 * _slow_platform
)

const ( /* platform delays */
	_no_delay_platform             = 0 /* use carefully; difficult to reincarnate on */
	_short_delay_platform          = TicksPerSecond
	_long_delay_platform           = 2 * TicksPerSecond
	_very_long_delay_platform      = 4 * TicksPerSecond
	_extremely_long_delay_platform = 8 * TicksPerSecond
)

const ( /* static platform flags */
	_platform_is_initially_active                            = iota /* otherwise inactive */
	...
	_platform_uses_native_polygon_heights      /* complicated interpretation; uses native polygon heights during automatic minmax calculation */
...
const ( /* dynamic platform flags */
	_platform_is_active          = iota /* otherwise inactive */
...
type endpointOwnerData struct {
	first_polygon_index, polygon_index_count int16
	first_line_index, line_index_count       int16
}

type staticPlatformData struct {
	typ                            int16
	speed, delay                   int16
	maximum_height, minimum_height WorldDistance /* if NONE then calculated in some reasonable way */
	static_flags                   uint32
	polygon_index                  int16
	tag                            int16
}
...
type platformData struct {
	...
	static_flags                                   uint32
	speed, delay                                   int16
	minimum_floor_height, maximum_floor_height     WorldDistance
	minimum_ceiling_height, maximum_ceiling_height WorldDistance
	polygon_index                                  int16
	dynamic_flags                                  cseries.Word
	floor_height, ceiling_height                   WorldDistance
	ticks_until_restart                            int16 /* if we're not moving but are active, this is our delay until we move again */
	endpoint_owners                                [MaximumVerticesPerPolygon]endpointOwnerData
